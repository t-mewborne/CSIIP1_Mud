...
"""
  ...
  @Test def testGetItem: Unit = {
    ...
    assertEquals(Item("sword", "A long pointy thing to slice and dice.", 1, 10), sword)
    assertEquals(None, room.getItem("sword"))
  }
  
  @Test def testDropItem: Unit = {
    assertEquals(None, room.getItem("Homework"))
    val item = Item("Homework", "Fun stuff you do for Dr. Lewis.", 1, 10)
    room.dropItem(item)
